...
while running:
    ...
    if (paddle_y <= ball_y + ball_radius <= paddle_y + paddle_height and
        ...
        # Tambahkan akselerasi berdasarkan skor
        ball_dx = int(ball_speed + score // 1) if ball_dx > 0 else int(-ball_speed - score // 1)
        ball_dy = int(ball_speed + score // 1) if ball_dy > 0 else int(-ball_speed - score // 1)
